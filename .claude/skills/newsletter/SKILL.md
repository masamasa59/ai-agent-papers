...
## 読みやすさの原則（最重要）
読者は「新しい言葉が多すぎる」「数字の凄さが分かる前に離脱する」で脱落する。Lilian Weng のブログ（明快な技術文章）に倣い、次を徹底する：
1. **定義より先に直感**：各号は `## まず、この分野を一言で` から始め、**専門語ゼロの比喩・目的**で全体像を語ってから用語を出す。
2. **用語は"目的"で説明してから使う**：例）「ハーネス＝モデルの周りを固める"段取り一式"（道具の呼び方・手順・記憶・やり直し方）」。英語ジャーゴンは初出で平易な日本語に言い換える（test-time→「本番中」、held-out→「見たことのない別タスク」、matched budget→「同じ計算量で公平に」、θ/Σ→「脳（重み）／周りの足場」）。
3. **新語の"数"を絞る**：1節に詰め込まない。同じ概念は一貫した平易な語で呼び続ける。`confidence cliff` 等の造語はそのまま出さず「回を重ねるほど審判が自信過剰で外す現象」のように説明する（必要なら括弧で原語）。
4. **数字は"何と比べてどうか"を先に置く**：`12%→50%` の前に「モデルを訓練せずに」「4倍以上」を添える。低い絶対値は「なぜ難しい課題か」を補う。
5. **なぜ → どう の順**：目的・効く理由を述べてから、仕組み・数式・数値を出す。
6. **明示的なつなぎ**：「まず」「ところが」「つまり」「だからこそ」で誘導。短い断定＋補足のリズム。
7. **論文紹介も"なぜ効くのか"から**：手法名の羅列・数値の抜き書きにしない。**背景 → 効く理由 → 結果 → 限界**の順。
8. 各号は `## この号のまとめ（何が確かで、何がまだ怪しいか）` で、確実な点／今月の注意／単発（要追試）を締めに整理する。
