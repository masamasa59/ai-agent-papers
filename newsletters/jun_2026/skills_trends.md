...
| 論文 | 数字 | 初見の読み方 |
|---|---|---|
| **SkillAdaptor** | WebShop +1.7pp／Claw-Eval +1.8（訓練不要） | 派手ではないが、**モデルを訓練せずスキルを直すだけ**で複数ベンチが底上げ |
| **OpenSkill** | closed-world 比 **+8.9pt**／他モデルへ 5.5–14.8% | **答えを見ずに**Webから学ぶだけで大幅増。しかも他モデルにも効く |
| **Notes2Skills** | **確信度100%一致**で149指令を保存 | 実験ノートの「たぶん」を「確定」に**取り違えない**＝科学応用で必須の忠実性 |
| **OpenClaw-Skill** | QwenClawBench 34.5→**44.9**（+10.4） | **複数モデルの集合知**でスキルを作ると、単一モデル製より約1割上 |
| **SkillComposer** | 実タスク劣化 **11pp**（SFTは27.5pp）／**154×省パラメータ** | わずか3.9Mの小型器が、**巨大なSFTより頑健**にスキルを選ぶ＝安くて崩れない |
| **EvoDS** | 再利用 **69%**／out-of-token 失敗を**全廃** | 覚えたスキルの7割を使い回し、**長い処理で文脈が溢れて落ちる事故をゼロに** |
