...
## なぜ注目か
コーディングは「エージェントが実社会のソフトを書く」最前線であり、**能力の伸び**と**運用の難しさ**が同時に露出する。6月は「AI が主たる生産者、人間は設計者・統治者・スチュワードになる」という展望（AI-Native SE）と、**その展望が長期信頼性・検証で頭打ちになる現実**が同居した。ここは [harness](../../architecture/harness.md)・[failure-attribution](../../capabilities/adaptation/failure-attribution.md) の潮流とも直結する。

## 数字で見るインパクト（各論文の本文より）

| 論文 | 主な結果 |
|---|---|
| **Agentic Software** | 分離タスク **>80% → 連続 38%** の性能崖／multi-agent で根因特定時間 **−93%** |
| **AI-Native SE**（review） | 有界タスクで **+55.8%** 速度／生成物の **約40%に脆弱性**／採用 76→84% だが信頼は低下 |
| **Evidence from Codex** | 週次アクティブ **5×**／8時間超タスク **2.1→25.6%**／**28.6%** が5+並列エージェント |
| **TMax** | Tmax-9B が Terminal-Bench 2.0 で **27%**／SWE-bench Verified 44→**53.5%**（+9.5） |
| **Socratic-SWE** | SWE-bench Verified **50.40%**（+7.80）／固定36k予算・3反復 |
| **SeeRepo** | vision単独は −13.6〜34.1pp だが、multimodal で **コスト −26%**・Pass@1 +0.4 |
| **FastContext** | 解決率 **+5.5%**・トークン **−60%**（※product IP 事由で取り下げ） |
