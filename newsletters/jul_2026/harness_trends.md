...
| 論文 | 数字 | 初見の読み方 |
|---|---|---|
| **TTHE** | text-to-SQL 12→**50%**（重み固定・ラベル無し） | 正答が**4倍以上**。モデルを一切触らず"周辺プログラム"だけで大化け |
| **Co-Harness** | 平均 **+20.4pp**／自律22版で **8.7× 高速化** | 足場と重みを同時に育てると複利で伸び、**人手なしにクラッシュ復旧まで** |
| **Living-Harness** | τ²-Bench **+10.07pp**／GLM-5 を **0→43%** | 対話タスクで一段上へ。凍結足場を移すと**全く解けなかったモデルが4割解ける** |
| **MemoHarness** | 精度で Codex 超え、コスト **$6.89**（Codex $10.28） | **安くて強い**。経験の使い回しで高精度と低コストを両立 |
| **HarnessBank** | 7ベンチ **+5.1〜15.4%**／利得はモデル特異 | 安定して伸びるが、**"万能な最適足場"は無く**モデルごとに別物 |
| **Better Harnesses** | 大型の **89.7%** を**コスト4%** | 良い足場を付ければ**小型モデルが大型の9割の性能を約1/25の値段**で |
| **Harness Handbook** | プラン勝率 38.3 vs 28.3・トークン **−12.7%** | "どこを直すか"を示すと、**より少ない計算でより良い改修** |
| **Rethinking Evaluation** | held-out 利得 **+0.6pt**／単純サンプリング **72.3% > 進化 67.4%** | ほぼ**据え置き**。公平に比べると"進化"は単に何度も試すより弱い＝過学習の警告 |
| **DataClawEval** | 最強でも **74.9/100**、エンジン間で 81→62 と激変 | まだ**2〜4割は失敗**。しかも**万能モデルは無く**得意分野がバラける |
| **Agent Harness Distillation** | 蒸留後 **+45pt** | 他エージェントの足場を**覗き見て真似ると激増**＝足場は盗める知財 |
